 ...
 * reader 读取是否结束
 */
const readIsOver = ref<boolean>(false)
...
async function scrollToBottomByThreshold() {
  if (!refWrapperContent.value)
    return
 ...
 */
function runReadBuffer(readCallback = () => {}, endCallback = () => {}) {
  ...
  >
    <transition name="fade">
      <n-float-button
        ...
        :right="30"
        color
        class="c-warning z-2 bg-#fff/80 transition-all-200 hover:bg-#fff/90"
    ...
    <template #icon>
      <div class="i-svg-spinners:3-dots-rotate" />
    </template>
    <!-- b="~ solid #ddd" -->
    <div
      flex="1 ~"
    ...
    >
      ...
      >
        ...
        >
          ...
          <template #icon>
            <n-icon>
              <div class="i-hugeicons:ai-chat-02" />
            </n-icon>
          </template>
        </n-empty>
        <div
