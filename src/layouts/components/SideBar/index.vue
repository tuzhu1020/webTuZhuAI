...
import { Dropdown as ADropdown, Input as AInput, Menu as AMenu, MenuItem as AMenuItem, Modal as AModal, Spin as ASpin, Tooltip as ATooltip } from 'ant-design-vue'
...
function handleClearChat() {
  modal.confirm({
    title: '删除所有历史对话',
    content: '如点击确认删除，当前账号的所有历史对话将被清空，无法找回。确认要删除所有历史对话吗？',
    okText: '确认删除',
    okButtonProps: {
      type: 'primary',
      danger: true,
  ...
  sideChatList.value.forEach((item) => {
    item.list.forEach((cur) => {
      if (cur.id === id) {
        cur._isEdit = true
        nextTick(() => {
          const ele = document.getElementById(cur._refContent) as HTMLInputElement
          ele.focus()
        })
      }
    })
...
watch(() => userStore.chatList, (newVal) => {
      ...
      <ASpin wrapper-class-name="a-spin-box" class="h-full" :spinning="loading">
        ...
        <div v-if="sideChatList.length" class="flex-1">
          <div v-for="(item, index) in sideChatList" :key="index" class="mb-24 px-10">
            ...
            <div
              v-for="cur in item.list"
              :key="cur.id"
              class="group relative box-border h-38 w-full cursor-pointer overflow-hidden rounded-12 px-10 transition-all duration-300 hover:bg-[var(--sidebar-hover)]"
              :class="curChatId === String(cur.id) ? 'bg-[var(--primary-bg-color)]' : ''"
              @click="router.push({ path: `/chat/${cur.id}` })"
            >
              ...
              <div
                v-if="!cur._isEdit"
                class="absolute right-12 top-50% z-2 hidden translate-y-[-50%] rounded-12 p4 group-hover:block hover:bg-white"
              >
      ...
      <div>
        <div class="p-x-12 p-y-10">
          <ADropdown placement="top" :trigger="['click']">
            <div
              class="flex cursor-pointer items-center justify-start rounded-12 p-8 transition-all duration-300 hover:bg-[var(--sidebar-hover)]"
            >
